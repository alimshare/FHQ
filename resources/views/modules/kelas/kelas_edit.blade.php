                      ...
                      <div class="row">
                        <div class="col-sm-12">
                          <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="hari" id="hari" placeholder="" required="" value="{{ $object->hari }}">
                          </div>
                        </div>
                      </div>
                      <!--/.row-->
...
@section('footer-script')
      <script type="text/javascript">
        $(function(){
          ...
          $.ajax({
             ...
             success:function(response){
                if (response.length) {
                  var opts =  '<option value="" disabled=""> -- Pilih semester -- </option>';
                  var current_semester = {{ $object->id_semester }};
                  $.each(response, function(index, value){
                    var selected = ( current_semester == value.id ? 'selected' : '');
          ...
          $.ajax({
             ...
             success:function(response){
                if (response.length) {
                  var opts =  '<option value="" disabled=""> -- Pilih level -- </option>';
          ...
          $.ajax({
             ...
             success:function(response){
                if (response.length) {
                  var opts =  '<option value="" disabled=""> -- Pilih pengajar -- </option>';
                  var current_pengajar = {{ $object->id_pengajar }};
                  $.each(response, function(index, value){
                    var selected = ( current_pengajar == value.id ? 'selected' : '');                    
        ...
        $("#form").on('submit', function(e){
            ...
            $.ajax({
               type:'PUT',
               url:'{{ url("api/kelas/update/".$object->id) }}',
